    ...
    <div class="bg-white rounded-lg shadow-md p-6 max-w-4xl mx-auto">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900">
                ...
                Novo Lote - {{ $empreendimento->nome }}
            </h1>
            <p class="text-gray-600 mt-1">Preencha os dados do lote</p>
        </div>

        <form method="POST" action="{{ route('lotes.store', $empreendimento->id) }}">
            @csrf
            ...
            <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="quadra_id" name="quadra_id" required
                                ...
                                @foreach($quadras as $quadra)
                                    <option value="{{ $quadra->id }}" {{ old('quadra_id') == $quadra->id ? 'selected' : '' }}>{{ $quadra->nome }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="lote_status_id" name="lote_status_id" required
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                @php
                                    $statusDisponivel = $status->firstWhere('nome', 'Disponível');
                                    $statusSelecionado = old('lote_status_id', $statusDisponivel ? $statusDisponivel->id : null);
                                @endphp
                                @foreach($status as $s)
                                    <option value="{{ $s->id }}" {{ $statusSelecionado == $s->id ? 'selected' : '' }}>{{ $s->nome }}</option>
                                @endforeach
                            </select>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div>
                            ...
                            <input type="number" id="frente" name="frente" value="{{ old('frente') }}" step="0.01" min="0"
                        ...
                        <div>
                            ...
                            <input type="number" id="fundo" name="fundo" value="{{ old('fundo') }}" step="0.01" min="0"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 disabled:cursor-not-allowed">
                        ...
                        <div>
                            ...
                            <input type="number" id="lateral_direita" name="lateral_direita" value="{{ old('lateral_direita') }}" step="0.01" min="0"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 disabled:cursor-not-allowed">
                        ...
                        <div>
                            ...
                            <input type="number" id="lateral_esquerda" name="lateral_esquerda" value="{{ old('lateral_esquerda') }}" step="0.01" min="0"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 disabled:cursor-not-allowed">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <select id="m2_tipo" name="m2_tipo" required
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                <option value="1" {{ old('m2_tipo', '1') == '1' ? 'selected' : '' }}>Calculado Automaticamente</option>
                                <option value="2" {{ old('m2_tipo') == '2' ? 'selected' : '' }}>Manual</option>
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="number" id="m2" name="m2" value="{{ old('m2') }}" step="0.01" min="0" required
                                ...
                                readonly>
                            <p class="text-xs text-gray-500 mt-1" id="m2-hint">Preencha as dimensões para calcular automaticamente</p>
                        ...
                        <div>
                            ...
                            <input type="number" id="valor_m2" name="valor_m2" value="{{ old('valor_m2', $empreendimento->valor_m2) }}" step="0.01" min="0" required
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                placeholder="{{ $empreendimento->valor_m2 ? 'Padrão: R$ ' . number_format($empreendimento->valor_m2, 2, ',', '.') : 'Digite o valor' }}">
                        ...
                        <div>
                            ...
                            <div class="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 min-h-[42px] flex items-center">
                                <span id="valor-display" class="text-gray-900 font-medium">R$ 0,00</span>
                            </div>
                ...
                <div>
                    ...
                    <textarea id="observacao" name="observacao" rows="4"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">{{ old('observacao') }}</textarea>
                    @error('observacao')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
            </div>

            <div class="mt-8 flex justify-end space-x-3">
                <a href="{{ route('lotes.index', $empreendimento->id) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-md text-sm font-medium">
                    Cancelar
                </a>
                <button type="submit" name="action" value="save_and_new" class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded-md text-sm font-medium">
                    <i class="fas fa-save mr-2"></i>
                    <i class="fas fa-plus mr-1"></i>
                    Salvar e Criar Novo
                </button>
                <button type="submit" name="action" value="save" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-md text-sm font-medium">
                    <i class="fas fa-save mr-2"></i>
                    Salvar Lote
                </button>
            </div>
