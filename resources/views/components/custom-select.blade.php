...
@php
    ...
    \Log::info("Custom Select DEBUG - ID: $id, Mode: $mode, LoadOnOpen: " . ($loadOnOpen ? 'true' : 'false') . ", AJAX URL: $ajaxUrl");
...
if (typeof window.buscarViaAjax === 'undefined') {
    window.buscarViaAjax = function(selectId, searchTerm, config) {
        console.log('[DEBUG] ═══════════════════════════════════════');
        console.log('[DEBUG] buscarViaAjax CHAMADO!');
        ...
        if (!ajaxUrl || ajaxUrl === 'null' || ajaxUrl === '') {
            console.error('[ERROR] AJAX URL não configurada para:', selectId, 'config:', config);
        ...
        console.log('[SUCCESS] Fazendo requisição AJAX para:', url.toString());
        ...
        .catch(error => {
            console.error('[ERROR] Erro ao buscar itens:', error);
            if (loadingEl) loadingEl.classList.add('hidden');
            if (emptyEl) {
...
if (typeof window.initializeCustomSelect === 'undefined') {
    ...
    console.log('[INIT] ============================================');
    console.log('[INIT] Inicializando:', selectId);
    console.log('[INIT] Mode recebido do Blade:', '{{ $mode }}');
    console.log('[INIT] LoadOnOpen do Blade:', {{ $loadOnOpen ? 'true' : 'false' }});
    console.log('[INIT] AJAX URL do Blade:', '{{ $ajaxUrl }}');
    console.log('[INIT] ============================================');
    ...
    function openDropdown() {
        console.log('[DEBUG] openDropdown chamado para:', selectId);
        if (!dropdown) {
            console.error('Dropdown not found for:', selectId);
            return;
        }

        console.log('[DEBUG] Opening dropdown for:', selectId);
        ...
        // MODO AJAX com loadOnOpen: Carregar itens ao abrir dropdown
        console.log('[DEBUG] Dropdown aberto - mode:', '{{ $mode }}', 'loadOnOpen:', {{ $loadOnOpen ? 'true' : 'false' }});
        ...
        if ('{{ $mode }}' === 'ajax') {
            ...
            console.log('[DEBUG] Itens já carregados:', existingItems.length);

            // Só carregar se não houver itens já carregados
            if (existingItems.length === 0) {
                console.log('[DEBUG] ▶ ETAPA 1: Mostrando "Carregando..."');

                // ETAPA 1: Mostrar loading IMEDIATAMENTE
                const loadingEl = document.getElementById(selectId + '_loading');
                const emptyEl = document.getElementById(selectId + '_empty');

                if (loadingEl) {
                    loadingEl.classList.remove('hidden');
                    console.log('[DEBUG] ✓ "Carregando..." visível');
                }
                if (emptyEl) {
                    emptyEl.classList.add('hidden');
                }

                console.log('[DEBUG] ▶ ETAPA 2: Preparando requisição AJAX...');

                // ETAPA 2: Obter URL e configuração
                const selectWrapper = document.querySelector('.custom-select-wrapper[data-select-id="' + selectId + '"]');
                const urlFromData = selectWrapper ? selectWrapper.dataset.ajaxUrl : null;
                const urlFromBlade = '{{ $ajaxUrl }}';
                const finalUrl = urlFromData || urlFromBlade;

                console.log('[DEBUG] URL encontrada:', finalUrl);

                if (!finalUrl || finalUrl === 'null' || finalUrl === '') {
                    console.error('[ERROR] URL AJAX não configurada!');
                    if (loadingEl) loadingEl.classList.add('hidden');
                    if (emptyEl) {
                        emptyEl.textContent = 'Erro: URL não configurada';
                        emptyEl.classList.remove('hidden');
                    }
                    return;
                }
                ...
                console.log('[DEBUG] ▶ ETAPA 3: Fazendo requisição AJAX...');

                // ETAPA 3: Fazer requisição AJAX (delay mínimo para garantir que o loading apareça)
                setTimeout(() => {
                    if (typeof window.buscarViaAjax === 'function') {
                        console.log('[DEBUG] ✓ Chamando buscarViaAjax');
                        window.buscarViaAjax(selectId, '', ajaxConfig);
                    } else {
                        console.error('[ERROR] Função buscarViaAjax não está definida!');
                        if (loadingEl) loadingEl.classList.add('hidden');
                        if (emptyEl) {
                ...
                console.log('[DEBUG] Itens já carregados, não fazendo nova requisição');
            }
        }
    ...
    input.addEventListener('focus', function(e) {
        console.log('[DEBUG] ►►► Input FOCUS event para:', selectId);
        e.stopPropagation();
        openDropdown();
    });
    ...
    input.addEventListener('click', function(e) {
        console.log('[DEBUG] ►►► Input CLICK event para:', selectId);
        e.stopPropagation();
        openDropdown();
    });
...
(function() {
    const selectId = '{{ $id }}';
    console.log('[DEBUG] ★ Iniciando inicialização do select:', selectId);
    function init() {
        const inputEl = document.getElementById(selectId + '_input');
        const dropdownEl = document.getElementById(selectId + '_dropdown');
        console.log('[DEBUG] Verificando elementos - input:', !!inputEl, 'dropdown:', !!dropdownEl);
        if (inputEl && dropdownEl) {
            console.log('[DEBUG] ✓ Elementos encontrados, inicializando custom select para:', selectId);
            initializeCustomSelect(selectId);
        } else {
            console.log('[DEBUG] Elementos não encontrados ainda, tentando novamente em 100ms...');
    ...
    if (document.readyState === 'loading') {
        console.log('[DEBUG] DOM ainda carregando, aguardando DOMContentLoaded...');
        document.addEventListener('DOMContentLoaded', init);
    } else {
        console.log('[DEBUG] DOM já pronto, inicializando imediatamente...');
        init();
    }
})();
</script>
