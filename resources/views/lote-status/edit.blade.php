...
@section('title', 'Editar Status de Lote')
    ...
    <div class="bg-white rounded-lg shadow-md p-6 max-w-2xl mx-auto">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900">
                ...
                Editar Status de Lote
            </h1>
            <p class="text-gray-600 mt-1">Atualize as informações do status</p>
        </div>

        <form method="POST" action="{{ route('lote-status.update', $loteStatus->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" id="nome" name="nome" value="{{ old('nome', $loteStatus->nome) }}" required
                ...
                <div>
                    ...
                    <select id="tipo" name="tipo" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                        <option value="1" {{ old('tipo', $loteStatus->tipo) == '1' ? 'selected' : '' }}>Disponível/Negociação/Reserva</option>
                        <option value="2" {{ old('tipo', $loteStatus->tipo) == '2' ? 'selected' : '' }}>Vendido</option>
                    </select>
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-3">
                        <input type="color" id="cor" name="cor" value="{{ old('cor', $loteStatus->cor) }}" required
                            class="h-10 w-20 border border-gray-300 rounded cursor-pointer">
                        <input type="text" id="cor-text" value="{{ old('cor', $loteStatus->cor) }}"
