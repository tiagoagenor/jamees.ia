    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                ...
                <form action="{{ route($tipo == 1 ? 'contas-a-pagar.update' : 'contas-a-receber.update', $movimentacao) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="entidade_id"
                                ...
                                @foreach($entidades as $entidade)
                                    <option value="{{ $entidade->id }}" {{ old('entidade_id', $movimentacao->entidade_id) == $entidade->id ? 'selected' : '' }}>
                                        {{ $entidade->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="situacao"
                                    id="situacao"
                                    required
                                    class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('situacao') border-red-500 @enderror">
                                <option value="">Selecione a situação</option>
                                <option value="1" {{ old('situacao', $movimentacao->situacao->value) == '1' ? 'selected' : '' }}>Pendente</option>
                                <option value="2" {{ old('situacao', $movimentacao->situacao->value) == '2' ? 'selected' : '' }}>Paga</option>
                                <option value="3" {{ old('situacao', $movimentacao->situacao->value) == '3' ? 'selected' : '' }}>Vencida</option>
                                <option value="4" {{ old('situacao', $movimentacao->situacao->value) == '4' ? 'selected' : '' }}>Cancelada</option>
                            </select>
                            @error('situacao')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number"
                                   name="juros"
                                   id="juros"
                                   value="{{ old('juros', $movimentacao->juros) }}"
                                   step="0.01"
                                   min="0"
                        ...
                        <div>
                            ...
                            <input type="number"
                                   name="desconto"
                                   id="desconto"
                                   value="{{ old('desconto', $movimentacao->desconto) }}"
                                   step="0.01"
                                   min="0"
