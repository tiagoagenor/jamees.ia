                            ...
                            @enderror
                                    ...
                                    <div>
                                        ...
                                        <textarea name="observacao"
                                                  ...
                                                  placeholder="Digite uma observação (opcional)">{{ old('observacao', $movimentacao->observacao) }}</textarea>
                                        @error('observacao')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <select name="juros_tipo"
                                    ...
                                    @php
                                        $jurosTipoValue = old('juros_tipo', $movimentacao->juros_tipo ?: 'fixo');
                                    @endphp
                                    <option value="fixo" {{ $jurosTipoValue == 'fixo' ? 'selected' : '' }}>Fixo</option>
                                    <option value="por_dia" {{ $jurosTipoValue == 'por_dia' ? 'selected' : '' }}>Por Dia</option>
                                </select>
                                @error('juros_tipo')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="juros_forma"
                                    ...
                                    @php
                                        $jurosFormaValue = old('juros_forma', $movimentacao->juros_forma ?: 'valor');
                                    @endphp
                                    <option value="valor" {{ $jurosFormaValue == 'valor' ? 'selected' : '' }}>Valor</option>
                                    <option value="porcentagem" {{ $jurosFormaValue == 'porcentagem' ? 'selected' : '' }}>Porcentagem</option>
                                </select>
                                @error('juros_forma')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                @php
                                    $jurosValue = old('juros');
                                    if ($jurosValue === null && $movimentacao->juros !== null) {
                                        // Usar number_format com muitas casas decimais e depois remover zeros à direita
                                        $jurosStr = number_format($movimentacao->juros, 10, '.', '');
                                        // Remover zeros à direita e ponto final se necessário
                                        $jurosValue = rtrim(rtrim($jurosStr, '0'), '.');
                                    }
                                @endphp
                                <input type="number"
                                       name="juros"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <select name="multa_forma"
                                    ...
                                    @php
                                        $multaFormaValue = old('multa_forma', $movimentacao->multa_forma ?: 'valor');
                                    @endphp
                                    <option value="valor" {{ $multaFormaValue == 'valor' ? 'selected' : '' }}>Valor</option>
                                    <option value="porcentagem" {{ $multaFormaValue == 'porcentagem' ? 'selected' : '' }}>Porcentagem</option>
                                </select>
                                @error('multa_forma')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                @php
                                    $multaValue = old('multa');
                                    if ($multaValue === null && $movimentacao->multa !== null) {
                                        // Usar number_format com muitas casas decimais e depois remover zeros à direita
                                        $multaStr = number_format($movimentacao->multa, 10, '.', '');
                                        // Remover zeros à direita e ponto final se necessário
                                        $multaValue = rtrim(rtrim($multaStr, '0'), '.');
                                    }
                                @endphp
                                <input type="number"
                                       name="multa"
                                        ...
                                        <div class="px-4 py-3 border-t border-gray-200 bg-gray-100 rounded-b-lg">
                                            <div class="text-center">
                                                ...
                                                <input type="text"
                                                       id="valor_total"
                                                       readonly
                                                       value="R$ {{ number_format($movimentacao->valor_total ?? ($movimentacao->valor + ($movimentacao->juros ?? 0) + ($movimentacao->multa ?? 0) - ($movimentacao->desconto ?? 0)), 2, ',', '.') }}"
                                                       class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm bg-gray-200 font-semibold text-gray-900 text-center">
                                                <input type="hidden" name="valor_total" id="valor_total_hidden" value="{{ $movimentacao->valor_total ?? ($movimentacao->valor + ($movimentacao->juros ?? 0) + ($movimentacao->multa ?? 0) - ($movimentacao->desconto ?? 0)) }}">
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
$(document).ready(function() {
    ...
    // Carregar entidade se já existir no old ou na movimentação
    @if(old('entidade_tipo') || $movimentacao->entidade_tipo)
        $entidadeTipoSelect.val('{{ old('entidade_tipo', $movimentacao->entidade_tipo) }}');
        $entidadeTipoSelect.trigger('change');
        @if(old('entidade_id') || $movimentacao->entidade_id)
            setTimeout(() => {
                $entidadeSelect.val('{{ old('entidade_id', $movimentacao->entidade_id) }}');
            }, 100);
        @endif
    @endif
    ...
    function alternarModo(isAtivo) {
        if (isAtivo) {
            ...
            if (!isInitialLoad || $('#descricao').val() || $('#plano_conta_id').val() || $('#valor').val()) {
            ...
            const temValoresParcelamento = $('#descricao_parcelamento').val() || 
                                          $('#plano_conta_id_parcelamento').val() || 
                                          $('#valor_parcelamento').val();
            
            if (temValoresParcelamento) {
    ...
    // Verificar estado inicial - NÃO executar alternarModo na edição para não sobrescrever valores
    // O modo normal já está visível e os campos já estão preenchidos
    // Apenas garantir que o modo parcelamento esteja oculto se o toggle não estiver marcado
    const $toggleParcelamento = getToggleParcelamento();
    if ($toggleParcelamento.length && !$toggleParcelamento.prop('checked')) {
        // Apenas garantir que o modo parcelamento esteja oculto
        $modoParcelamento.addClass('hidden');
        $modoNormal.removeClass('hidden');
        // Marcar que o carregamento inicial foi concluído
        isInitialLoad = false;
    } else if ($toggleParcelamento.length && $toggleParcelamento.prop('checked')) {
        // Se o toggle estiver marcado, executar alternarModo
        alternarModo(true);
        // Marcar que o carregamento inicial foi concluído
        isInitialLoad = false;
    } else {
        // Se não houver toggle, marcar que o carregamento inicial foi concluído
        isInitialLoad = false;
    }
    
    // Marcar que o carregamento inicial foi concluído após um pequeno delay
    setTimeout(() => {
        isInitialLoad = false;
    }, 500);
