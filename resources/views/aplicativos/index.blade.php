...
<div class="container mx-auto px-4 py-8">
    ...
    <!-- Seção Topo: Lançamentos e Em Alta -->
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
            ...
            <div class="p-6">
                <div class="slider-container relative">
                    <div class="slider-wrapper overflow-hidden rounded-lg">
                        <div class="slider-track flex transition-transform duration-500 ease-in-out" id="sliderTrack">
                            @forelse($lancamentos as $index => $lancamento)
                                <div class="slider-slide min-w-full">
                                    <div class="relative h-64 bg-gray-100 rounded-lg overflow-hidden cursor-pointer border border-gray-200" onclick="abrirModalDetalhes(@json($lancamento))">
                                        @if($lancamento->imagem)
                                            <img src="{{ $lancamento->imagem }}" alt="{{ $lancamento->nome }}" class="w-full h-full object-cover">
                                        @else
                                            <div class="text-gray-400 text-center flex items-center justify-center h-full">
                                                <i class="fas fa-mobile-alt text-6xl opacity-50"></i>
                                            </div>
                                        @endif
                                        <div class="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end">
                                            <div class="p-4 w-full">
                                                <h3 class="text-white font-bold text-lg mb-1">{{ $lancamento->nome }}</h3>
                                                <p class="text-white text-sm opacity-90">{{ \Illuminate\Support\Str::limit($lancamento->descricao ?? '', 60) }}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            @empty
                                <div class="min-w-full">
                                    <div class="h-64 bg-gray-100 rounded-lg flex items-center justify-center border border-gray-200">
                                        <p class="text-gray-500">Nenhum lançamento disponível</p>
                                    </div>
                                </div>
                            @endforelse
                        </div>
                    ...
                    @if($lancamentos->count() > 1)
                        <button onclick="sliderPrev()" class="absolute left-2 top-1/2 -translate-y-1/2 bg-white hover:bg-gray-50 text-gray-800 rounded-full p-2 shadow-md border border-gray-200 transition-all">
                            <i class="fas fa-chevron-left"></i>
                        </button>
                        <button onclick="sliderNext()" class="absolute right-2 top-1/2 -translate-y-1/2 bg-white hover:bg-gray-50 text-gray-800 rounded-full p-2 shadow-md border border-gray-200 transition-all">
                            <i class="fas fa-chevron-right"></i>
                        </button>
                        <div class="flex justify-center mt-4 space-x-2" id="sliderDots">
                            @foreach($lancamentos as $index => $lancamento)
                                <button onclick="goToSlide({{ $index }})" class="slider-dot w-2 h-2 rounded-full {{ $index === 0 ? 'bg-blue-600' : 'bg-gray-300' }} transition-all"></button>
                            @endforeach
                        </div>
                    @endif
                </div>
        ...
        <div class="bg-white shadow rounded-lg overflow-hidden">
            ...
            <div class="p-6">
                <div class="space-y-3">
                    @forelse($emAlta as $aplicativo)
                        <div class="flex items-center p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer border border-gray-200" onclick="abrirModalDetalhes(@json($aplicativo))">
                            <div class="flex-shrink-0 w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center mr-4 border border-blue-200">
                                @if($aplicativo->imagem)
                                    <img src="{{ $aplicativo->imagem }}" alt="{{ $aplicativo->nome }}" class="w-full h-full object-cover rounded-lg">
                                @else
                                    <i class="fas fa-mobile-alt text-blue-600 text-xl"></i>
                                @endif
                            </div>
                            <div class="flex-1 min-w-0">
                                <h3 class="text-sm font-semibold text-gray-900 truncate">{{ $aplicativo->nome }}</h3>
                                <p class="text-xs text-gray-500 truncate">{{ $aplicativo->categoria ?? 'Sem categoria' }}</p>
                            </div>
                    ...
                    @empty
                        <div class="text-center py-8">
                            ...
                            <p class="text-gray-500 text-sm">Nenhum aplicativo em alta</p>
                        </div>
                    @endforelse
                </div>
            </div>
...
<script>
let currentSlide = 0;
const totalSlides = {{ $lancamentos->count() }};
