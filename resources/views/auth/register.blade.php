    ...
    <div class="max-w-md w-full space-y-8">
        ...
        <form class="mt-8 space-y-6" method="POST" action="{{ route('register') }}">
            ...
            <div class="rounded-md shadow-sm -space-y-px">
                <div>
                    <label for="empresa_nome" class="sr-only">Nome da Empresa</label>
                    <input id="empresa_nome" name="empresa_nome" type="text" required
                           class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                           placeholder="Nome da Empresa" value="{{ old('empresa_nome') }}">
                    @error('empresa_nome')
                ...
                <div>
                    ...
                    <input id="telefone" name="telefone" type="tel" required
                           class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                           placeholder="Telefone (ex: 31996241675)" value="{{ old('telefone') }}">
                    @error('telefone')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
