...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600 mt-2">Atualize as informações da estrutura DRE</p>
            </div>
            <div class="flex space-x-3">
                <a href="{{ route('dre.show', $dre) }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-md text-sm font-medium">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('dre.update', $dre) }}" method="POST" id="dreForm">
            @csrf
            @method('PUT')
                        ...
                        <select id="tipo"
                            ...
                            @foreach($tipos as $tipo)
                                <option value="{{ $tipo->value }}"
                                        {{ old('tipo', $dre->tipo->value) == $tipo->value ? 'selected' : '' }}
                        ...
                        <select id="dre_id"
                            ...
                            @foreach($dresPai as $drePai)
                                <option value="{{ $drePai->id }}"
                                        {{ old('dre_id', $dre->dre_id) == $drePai->id ? 'selected' : '' }}>
                        ...
                        <div class="flex items-center space-x-4">
                            <label class="flex items-center">
                                <input type="radio"
                                       name="status"
                                       value="1"
                                       {{ old('status', $dre->status ? '1' : '0') == '1' ? 'checked' : '' }}
                            ...
                            <label class="flex items-center">
                                <input type="radio"
                                       name="status"
                                       value="0"
                                       {{ old('status', $dre->status ? '1' : '0') == '0' ? 'checked' : '' }}
                                       class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300">
