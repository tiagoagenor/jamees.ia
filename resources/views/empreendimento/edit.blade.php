...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-lg shadow-md p-6 max-w-4xl mx-auto">
        <div class="mb-6">
            ...
            <p class="text-gray-600 mt-1">Atualize os dados do empreendimento</p>
        </div>

        <form method="POST" action="{{ route('empreendimentos.update', $empreendimento->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" id="nome" name="nome" value="{{ old('nome', $empreendimento->nome) }}" required
                        ...
                        <div>
                            ...
                            <input type="number" id="zoom_default" name="zoom_default" value="{{ old('zoom_default', $empreendimento->zoom_default ?? 180) }}" min="50" max="300"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                placeholder="180">
                        ...
                        <div>
                            ...
                            <select id="quadra_numeracao_tipo" name="quadra_numeracao_tipo" required
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                <option value="1" {{ old('quadra_numeracao_tipo', $empreendimento->quadra_numeracao_tipo ?? '1') == '1' ? 'selected' : '' }}>Numérica (1, 2, 3, 4...)</option>
                                <option value="2" {{ old('quadra_numeracao_tipo', $empreendimento->quadra_numeracao_tipo ?? '1') == '2' ? 'selected' : '' }}>Alfanumérica (A, B, C, D...)</option>
                            </select>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" id="valor_m2" name="valor_m2" value="{{ old('valor_m2', $empreendimento->valor_m2) }}" step="0.01" min="0"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <div>
                            ...
                            <input type="number" id="maximo_parcelas" name="maximo_parcelas" value="{{ old('maximo_parcelas', $empreendimento->maximo_parcelas) }}" min="1"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="sinal" name="sinal" required
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                <option value="1" {{ old('sinal', $empreendimento->sinal) == '1' ? 'selected' : '' }}>Sim</option>
                                <option value="2" {{ old('sinal', $empreendimento->sinal) == '2' ? 'selected' : '' }}>Não</option>
                            </select>
                            @error('sinal')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>

                        <div id="sinal-tipo-container" class="{{ old('sinal', $empreendimento->sinal) == '1' ? '' : 'hidden' }}">
                            ...
                            <select id="sinal_tipo" name="sinal_tipo"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                <option value="1" {{ old('sinal_tipo', $empreendimento->sinal_tipo) == '1' ? 'selected' : '' }}>Fixo</option>
                                <option value="2" {{ old('sinal_tipo', $empreendimento->sinal_tipo) == '2' ? 'selected' : '' }}>Porcentagem</option>
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="juros_forma" name="juros_forma"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                <option value="valor" {{ old('juros_forma', $empreendimento->juros_forma ?? 'valor') == 'valor' ? 'selected' : '' }}>Valor</option>
                                <option value="porcentagem" {{ old('juros_forma', $empreendimento->juros_forma ?? 'valor') == 'porcentagem' ? 'selected' : '' }}>Porcentagem</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" id="juros" name="juros" value="{{ old('juros', $empreendimento->juros ?? 0) }}" step="any"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                placeholder="0.00">
                        ...
                        <div>
                            ...
                            <select id="multa_forma" name="multa_forma"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                <option value="valor" {{ old('multa_forma', $empreendimento->multa_forma ?? 'valor') == 'valor' ? 'selected' : '' }}>Valor</option>
                                <option value="porcentagem" {{ old('multa_forma', $empreendimento->multa_forma ?? 'valor') == 'porcentagem' ? 'selected' : '' }}>Porcentagem</option>
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" id="multa" name="multa" value="{{ old('multa', $empreendimento->multa ?? 0) }}" step="any"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                placeholder="0.00">
                        ...
                        <div>
                            ...
                            <input type="number" id="juros_por_parcela" name="juros_por_parcela" value="{{ old('juros_por_parcela', $empreendimento->juros_por_parcela ?? 0) }}" step="any"
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                                placeholder="0.00">
